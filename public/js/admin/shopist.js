...
shopist.event =
  {
    ...
    init_dropzone: function ()
    {
      ...
      if ($('.dropzone_banner1_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner1_upload', paramName: 'banner1_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }

      if ($('.dropzone_banner2_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner2_upload', paramName: 'banner2_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }

      if ($('.dropzone_banner3_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner3_upload', paramName: 'banner3_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }

      if ($('.dropzone_banner4_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner4_upload', paramName: 'banner4_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }

     
      if ($('.dropzone_banner5_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner5_upload', paramName: 'banner5_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }
      if ($('.dropzone_banner6_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner6_upload', paramName: 'banner6_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }
      if ($('.dropzone_banner7_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner7_upload', paramName: 'banner7_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }

      if ($('.dropzone_banner8_upload').length > 0)
      {

        new customDropZone({ base_url: baseUrl, url: baseUrl + "/upload/product-related-image", token: token, uploader: 'dropzone_banner8_upload', paramName: 'banner8_image', acceptedFiles: "image/*", uploadMultiple: false, maxFiles: 1 });
      }
...
shopist.normalFunction =
  {
    ...
    update_frontend_img_link_in_json_data: function (id,value)
    {
      if ($('#_frontend_images_json').length > 0 && $('#_frontend_images_json').val())
      {
        var parseJson = JSON.parse($('#_frontend_images_json').val());

        var index = parseJson.header_slider_images_and_text.slider_images.findIndex((ele) =>
        {
          return ele.id == id;
        });
        parseJson.header_slider_images_and_text.slider_images[index].link = value;
        $('#_frontend_images_json').val(JSON.stringify(parseJson));
      }
    },update_product_img_alt_in_json_data: function (where,id, value)
      {
        if ($('#hf_uploaded_all_images').length > 0 && $('#hf_uploaded_all_images').val())
        {
          var parseJson = JSON.parse($('#hf_uploaded_all_images').val());
    
          if (where == 'image-gallery')
          {
            var gallery = parseJson.product_gallery_images;
              var index = parseJson.product_gallery_images.findIndex((ele) =>
              {
                return ele.id == id;
              });
              parseJson.product_gallery_images[index].alt = value;
          } else if (where == 'product-image')
          {
            parseJson.product_image_alt = value;
          }else if(where == 'shop-banner'){
            parseJson.shop_banner_image_alt = value;
          }
          $('#hf_uploaded_all_images').val(JSON.stringify(parseJson));
        }
      },
      ...
      init: function () 
      {
        ...
        this.on("success", function (file, response)  
        {
          if (response.status === 'success')
          {
            ...
            } else if (name == "banner1_image")
            {
              $(".banner1-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner1Uploader').modal('hide');
              $('.banner1-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner1_input').val("/public/uploads/" + response.name);
            } else if (name == "banner2_image")
            {
              $(".banner2-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner2Uploader').modal('hide');
              $('.banner2-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner2_input').val("/public/uploads/" + response.name);
            } else if (name == "banner3_image")
            {
              $(".banner3-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner3Uploader').modal('hide');
              $('.banner3-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner3_input').val("/public/uploads/" + response.name);
            } else if (name == "banner4_image")
            {
              $(".banner4-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner4Uploader').modal('hide');
              $('.banner4-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner4_input').val("/public/uploads/" + response.name);
            } else if (name == "banner5_image")
            {
              $(".banner5-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner5Uploader').modal('hide');
              $('.banner5-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner5_input').val("/public/uploads/" + response.name);
            } else if (name == "banner6_image")
            {
              $(".banner6-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner6Uploader').modal('hide');
              $('.banner6-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner6_input').val("/public/uploads/" + response.name);
            } else if (name == "banner7_image")
            {
              $(".banner7-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner7Uploader').modal('hide');
              $('.banner7-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner7_input').val("/public/uploads/" + response.name);
            } else if (name == "banner8_image")
            {
              $(".banner8-uploaded-image img").attr("src", base_url + "/public/uploads/" + response.name);
              $('#banner8Uploader').modal('hide');
              $('.banner8-sample-img').hide();
              $('.banner-uploaded-image').show();
              $('#banner8_input').val("/public/uploads/" + response.name);
            }
                ...
                for (var count = 0; count < parseResponse.length; count++)
                {
                  ...
                  if ($('.product-uploaded-gallery-image').find('.' + addExtraId).length == 0)
                  {
                    if ($('.gallery-image-single-container').length > 0)
                    {
                      ...
                      $(`.${addExtraId} img`).after(`<div id="${id}"  ><input value='' class="form-control" placeholder='alt' onkeyup="setAlt('image-gallery','${id}',this.value)" ></div>`)
                    }
                    else
                    {
                      ...
                      $(`.${addExtraId} img`).after(`<div id="${id}"  ><input value='' placeholder='alt' class="form-control" onkeyup="setAlt('image-gallery','${id}',this.value)" ></div>`);
                    }
                    ...
                    if ($('#hf_uploaded_all_images').length > 0)
                    {
                      shopist.normalFunction.createJsonDataForAllProductImage('product_gallery_images', { id: id, url: '/public/uploads/' + parseResponse[count], alt: '' });
                    }
                  }
                }
              }
                ...
                for (var count = 0; count < parseResponse.length; count++)
                {
                  ...
                  if ($('.uploaded-header-slider-images .uploaded-slider-images').find('.' + addExtraId).length == 0)
                  {
                    if ($('.header-slider-image-single-container').length > 0)
                    {
                      ...
                      $('.slider_links:last div:last').after(`<div id="${id}"  ><label>slider link </label><input value='' class="form-control" onkeyup="setLink('${id}',this.value)" ></div>`);
                    }
                    else
                    {
                      ...
                      $('.slider_links div:last').append(`<div id="${id}"  ><label>slider link </label><input value='' class="form-control" onkeyup="setLink('${id}',this.value)"></div>`);
                    }
                    ...
                    {
                      shopist.normalFunction.createJsonDataForAllFrontendImage('header_slider_images', { id: id, url: '/public/uploads/' + parseResponse[count],link:"" });
                    }
