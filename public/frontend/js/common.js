...
$(document).ready(function ()
{
  ...
  btnback = $("#checkout_page .back");
  btnsubmit = $("#checkout_page .submit");

  var form = $("#checkout_page #checkout_form");

  // Init buttons and UI
  widget.not(':eq(0)').hide();
  hideButtons(currentProgress);
  ...
  function cartTotal()
  {
    ...
    function payWithPaystack()
    {
      ...
      var handler = PaystackPop.setup({
        key: 'pk_test_012ab2f60005472090514ee57b86e56a732d9d18', // Replace with your public key
        email: email,
        ...
        callback: function (response)
        {
          ...
          $("#checkout_page #proceed").val("checkout_proceed");
    ...
    {
      e.preventDefault();
      if ($("input[name='payment_option']:checked").val() == "paystack")
      {
        payWithPaystack();
      }
      else
      ...
      if (checkoutStepValidation())
      {
        ...
        if (currentView < widget.length)
        {
          ...
          if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'login_user' && !is_enable_selected && $('#is_user_login').val() == false)
          {
            ...
            currentView = 3;
            currentProgress = 4;
            widget.not(':eq(' + currentView + ')').hide();
          ...
          else if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'login_user' && !is_enable_selected && $('#is_user_login').val() == true)
          {
            ...
            var login_user_address_index = 3;
            widget.not(':eq(' + login_user_address_index + ')').hide();
            is_enable_selected = true;
            currentView = 3;
            currentProgress = 4;
            console.log(parseInt($('#checkout_page .checkout-content').find('#payment').index()));
          ...
          else if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'guest' && !is_enable_selected)
          {
            ...
            is_enable_selected = true;
            current = 4;
          } else if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'guest' && is_enable_selected && currentView == 2)
          {
            var get_guest_user_address_index = parseInt($('#checkout_page .checkout-content').find('#guest_user_address').index()) - 1;
            var get_payment_index = parseInt($('#checkout_page .checkout-content').find('#payment').index()) - 1;
            currentView = 4;
    ...
    btnback.click(function (e)
    {
      ...
      if (currentView > 0)
      {
        ...
        if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'login_user' && $('#is_user_login').val() == true && currentView == 3)
        {
        ...
        } else if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'login_user' && $('#is_user_login').val() == false && currentView == 3)
        {
          currentView = 1;
          currentProgress = 2;
          widget.not(':eq(' + currentView + ')').hide();
        ...
        else if ($('#selected_user_mode').val().length > 0 && $('#selected_user_mode').val() == 'guest' && currentView == 4)
        {
          currentView = 2;
          currentProgress = 3;
          widget.not(':eq(' + currentView + ')').hide();
        ...
        else
        {
          if (currentView == 1)
          {
            if ($('#selected_user_mode').val() == 'login_user')
            {
              $("input[name='user_checkout_complete_type']").prop('checked', false);
            }
          }
    ...
    // if ($('.todays-deal-items').length > 0)
    // {
    //   $('.todays-deal-items').slick({
    //     infinite: true,
    //     slidesToShow: 5,
    //     slidesToScroll: 1,
    //     autoplay: true,
...
function hideButtons(current)
{
  ...
  if (current <= limit && visible_step.attr('id') != 'order_notes') { btnnext.show(); btnback.show() };
  if (current == limit && visible_step.attr('id') == 'order_notes') { btnnext.hide(); btnback.show(); btnsubmit.show(); }
}

var delivery_error_message = "";
function checkoutStepValidation()
{
  ...
  else if (visible_step.attr('id') && visible_step.attr('id') == 'guest_user_address')
  {
    ...
    if (verifyDelivery() == false)
    {
      errorStr.push("set valid delivery datas");
    }
  ...
  else if (visible_step.attr('id') && visible_step.attr('id') == 'login_user_address')
  {
    ...
    } else if (verifyDelivery('user') == false)
    {
      $('#' + visible_step.attr('id')).find('.user-address-content').before(msgStr);
      $('#' + visible_step.attr('id')).find('.error-msg').html(delivery_error_message);
      document.getElementById("err").scrollIntoView();
      returnVal = false;
    }
  }
...
function verifyDelivery(userMode = '')
{
  ...
  if (delivery_zone.length > 0 && delivery_area.length > 0 && delivery_date.length > 0 && delivery_time.length > 0)
  {
    ...
    {
      delivery_error_message = "please select valid date and time";
      return false;
    } else
    {
      return true;
  ...
  {
    delivery_error_message = "please fill in your delivery form appropriately";
    return false;
  }
