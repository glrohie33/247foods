...
class SettingsController extends Controller
{
  ...
  public function settingsTopCatContent()
  {
    $data = array();

    $data = $this->classCommonFunction->commonDataForAllPages();
    $data['menu_html'] = $this->topCatContent();
    $is_vendor = is_vendor_login();
    $sidebar['is_vendor_login'] = $is_vendor;
    $data['sidebar_data'] = $sidebar;
    return view('pages.admin.settings.top-cat-content', $data);
  }
  ...
  public function topCatSaveContent(Request $request)
  {
    if ($request->isMethod('post') && $request->has('top_cat_menu')) {
      $option = Option::where('option_name', '_option_top_category')->take(1)->first();
      if (boolval($option)) {
        $option->option_value = json_encode($request->Input('top_cat_menu'));
        $option->save();
      }
    }
    return Redirect()->back();
  }
  public function topCatContent()
  {
    $html = "";
    $parent_categories = $this->product->get_parent_categories(0, 'product_cat');
    ...
    if (count($parent_categories) > 0) {
      foreach ($parent_categories as $cat) {
        $checked = (in_array($cat['term_id'], $topCat)) ? 'checked="checked"' : "";
        $html .=  '<li class="ui-state-default">';
        $html .=  '<div class="input-group mb-3">';
        $html .=  '<div class="input-group-prepend">';
        $html .=  '<span class="input-group-text" id="basic-addon1">
        <input type="checkbox" name="top_cat_menu[]" ' . $checked . ' class="menu-checkbox" value="' . $cat['term_id'] . '"></span>';
        $html .=  '</div>';
        $html .=  '<label class="form-control">' . $cat['name'] . ' &nbsp;&nbsp; (' . Lang::get('admin.parent_cat_trace_label') . ')</label>';
        $html .=  '</div></li>';
      }
