...
class FrontendAjaxController extends Controller
{
  ...
  public function searchProductsByFilterWithName()
  {
    if (Request::ajax()) {
      $input = Request::all();

      $data = Product::where(['status' => 1])->where('title', 'LIKE', '%' . $input['search'] . '%')->take(10)->get();
  ...
  public function productRemoveFromCart($item_id)
  {

    if ($item_id) {
      if ($this->classCommonFunction->remove_item($item_id)) {
  ...
  {
    return  Product::find($id)->product_weight;
  }

  public function cartTotal()
  {
    $return['status'] = true;
    $return['amount'] = $this->classCommonFunction->get_cart_total();
  ...
  public function cartTotalUpdateUsingShippingMethod()
  {
    if (Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')) {
      ...
      if ($input['data'] == 'flat_rate') {
        ...
        } elseif ($shipping_details['local_delivery']['fee_type'] == 'per_weight') {
          $shipping_cost = Cart::getLocalDeliveryShippingPerProductWeight();
        }
      }
  ...
  public function storeSubscriptionData()
  {
    if (Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')) {
      ...
      if ($input['type'] == 'custom') {
        ...
        $subscriptions->name    =   (isset($input['name'])) ? urldecode($input['name']) : '';
  ...
  public function applyUserCoupon()
  {
    ...
    if (Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')) {
      ...
      } elseif (!empty($response) && $response == 'discount_from_shipping') {
        return response()->json(array('success' => true, 'success_type' => 'discount_from_shipping', 'discount_price' => price_html(get_product_price_html_by_filter(Cart::couponPrice(), get_frontend_selected_currency())), 'grand_total' => price_html(get_product_price_html_by_filter(Cart::getCartTotal(), get_frontend_selected_currency()))));
      } elseif (!empty($response) && $response == 'percentage_discount_from_shipping') {
        return response()->json(array('success' => true, 'success_type' => 'percentage_discount_from_shipping', 'discount_price' => price_html(get_product_price_html_by_filter(Cart::couponPrice(), get_frontend_selected_currency())), 'grand_total' => price_html(get_product_price_html_by_filter(Cart::getCartTotal(), get_frontend_selected_currency()))));
      } elseif (!empty($response) && $response == 'discount_error') {
        return response()->json(array('error' => true, 'error_type' => 'discount_error'));
      }
    }
  }
  ...
  {
    if (Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')) {
      $items = json_encode($this->classCommonFunction->cartItems());
      return response()->json(array('status' => 'success', 'type' => 'mini_cart_data', 'items' => $items));
    }
  }
