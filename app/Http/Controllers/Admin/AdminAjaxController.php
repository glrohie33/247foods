...
class AdminAjaxController extends Controller
{
   ...
   public function saveRelatedImage()
  {
    if (Request::isMethod('post') && Request::ajax()) {
      if (Session::token() == Request::header('X-CSRF-TOKEN')) {
        ...
        } elseif (isset($input['banner1_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner2_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner3_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner4_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner5_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner6_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner7_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        } elseif (isset($input['banner8_image'])) {
          $rules = array(
            'banner1_image' => 'image',
          );
        ...
        if ($validation->fails()) {
          ...
          if (isset($input['product_image'])) {
            ...
            $image = Request::file('banner3_image');
            $fileName = time() . "-" . "w-1170-h-150-" . $image->getClientOriginalName();
            $height = 521;
            $width  = 600;
          } elseif (isset($input['banner4_image'])) {
            $image = Request::file('banner4_image');
            $fileName = time() . "-" . "w-1170-h-150-" . $image->getClientOriginalName();
            $height = 521;
            $width  = 600;
          } elseif (isset($input['banner5_image'])) {
            $image = Request::file('banner5_image');
            $fileName = time() . "-" . "w-1170-h-150-" . $image->getClientOriginalName();
            $height = 500;
            $width  = 1144;
          } elseif (isset($input['banner6_image'])) {
            $image = Request::file('banner6_image');
            $fileName = time() . "-" . "w-1170-h-150-" . $image->getClientOriginalName();
            $height = 500;
            $width  = 1144;
          } elseif (isset($input['banner7_image'])) {
            $image = Request::file('banner7_image');
            $fileName = time() . "-" . "w-1170-h-150-" . $image->getClientOriginalName();
            $height = 500;
            $width  = 1144;
          } elseif (isset($input['banner8_image'])) {
            $image = Request::file('banner8_image');
            $fileName = time() . "-" . "w-1170-h-150-" . $image->getClientOriginalName();
            $height = 500;
            $width  = 1144;
          } elseif (isset($input['cat_thumbnail_image'])) {
          ...
          if ($image->getClientOriginalExtension() === 'gif') {
            ...
            $moved = move_uploaded_file($image->getPathName(), $path);
            if ($moved) {
  ...
  function saveBanners()
  {
    ...
    if (Request::isMethod('post') && Session::token() == Request::input('_token')) {
      ...
      if (isset($input['banner1']) && !empty($input['banner1'])) {
        $image = $input['banner1'];
        $url = "";
        if (isset($input['banner1_url']) && !empty($input['banner1_url'])) {
          $url = $input['banner1_url'];
        }
        $banner1["image"] = $image;
        $banner1['url']  = $url;
      }

      if (isset($input['banner2']) && !empty($input['banner2'])) {
        $image = $input['banner2'];
        $url = "";
        if (isset($input['banner2_url']) && !empty($input['banner2_url'])) {
          $url = $input['banner2_url'];
        }
        $banner2["image"] = $image;
        $banner2['url']  = $url;
      }
      if (isset($input['banner3']) && !empty($input['banner3'])) {
        $image = $input['banner3'];
        $url = "";
        if (isset($input['banner3_url']) && !empty($input['banner3_url'])) {
          $url = $input['banner3_url'];
        }
        $banner3["image"] = $image;
        $banner3['url']  = $url;
      }
      ...
      if (isset($input['banner4']) && !empty($input['banner4'])) {
        $image = $input['banner4'];
        $url = "";
        if (isset($input['banner4_url']) && !empty($input['banner4_url'])) {
          $url = $input['banner4_url'];
        }
        $banner4["image"] = $image;
        $banner4['url']  = $url;
      }
      if (isset($input['banner5']) && !empty($input['banner5'])) {
        $image = $input['banner5'];
        $url = "";
        if (isset($input['banner5_url']) && !empty($input['banner5_url'])) {
          $url = $input['banner5_url'];
        }
        $banner5["image"] = $image;
        $banner5['url']  = $url;
      }
      if (isset($input['banner6']) && !empty($input['banner6'])) {
        $image = $input['banner6'];
        $url = "";
        if (isset($input['banner6_url']) && !empty($input['banner6_url'])) {
          $url = $input['banner6_url'];
        }
        $banner6["image"] = $image;
        $banner6['url']  = $url;
      }

      if (isset($input['banner7']) && !empty($input['banner7'])) {
        $image = $input['banner7'];
        $url = "";
        if (isset($input['banner7_url']) && !empty($input['banner7_url'])) {
          $url = $input['banner7_url'];
        }
        $banner7["image"] = $image;
        $banner7['url']  = $url;
      }

      if (isset($input['banner8']) && !empty($input['banner8'])) {
        $image = $input['banner8'];
        $url = "";
        if (isset($input['banner8_url']) && !empty($input['banner8_url'])) {
          $url = $input['banner8_url'];
        }
        $banner8["image"] = $image;
        $banner8['url']  = $url;
      }

      $banners = array($banner1, $banner2, $banner3, $banner4, $banner5, $banner6, $banner7, $banner8);

      $data = array("option_value" => serialize($banners));
      if (Option::where("option_name", "_home_banner")->update($data)) {
        return Response::json(array("status" => true, 'messages' => 'settings saved'));
      }


      // if ($option->save()) {
      //   return Response::json(array("status" => true, 'messages' => 'Delivery zone has been added'));
      // } else {
  ...
  function saveDeliveryArea()
  {
    if (Request::isMethod('post') && Session::token() == Request::input('_token')) {
      ...
      $messages = [
        'zone_name.required' => "Zone Name is compulsory",
        'zone_areas.required' => "Zone Area is Required",
        'same_day.required' => "Same Day field is Required",
        'weekend.required' => "Week End is Required",
        'min_days.required' => "Minimum Days is Required",
        'open_hour.required' => "Open Hour is Required",
        'close_hour.required' => "Close Hour is Required"
      ];

      $validator = Validator::make($data, $rules, $messages);
      if ($validator->fails()) {
        $return = array('success' => false, 'messages' => json_encode($validator->errors()));
        return Response::json($return);
      } else {
        $deliveryArea = new DeliveryArea();
        $deliveryArea->delivery_zone = Request::input('zone_name');
        $deliveryArea->zone_areas = Request::input('zone_areas');
        $deliveryArea->sameday_delivery = Request::input('same_day');
        $deliveryArea->weekend_delivery = Request::input('weekend');
        $deliveryArea->min_days = Request::input('min_days');
        $deliveryArea->start_hour = Request::input('open_hour');
        $deliveryArea->end_hour = Request::input('close_hour');
  ...
  function editDeliveryArea()
  {
    if (Request::isMethod('post') && Session::token() == Request::input('_token') && !empty(Request::input('zone_id'))) {
      $data = Request::input();
      $rules = [
        'zone_name'                 => 'required',
        'zone_areas'                => 'required',
      ...
      if ($validator->fails()) {
        ...
        if ($deliveryArea->save()) {
          return Response::json(array("status" => true, 'messages' => 'Delivery zone has been edited'));
        } else {
          die("there is an error");
        }
      }
