...
class LoginController extends Controller
{
  ...
  public function goToFrontendLoginPage()
  {
    ...
    return view('pages.auth.top-login')->with($data);
  }
    ...
    if (Request::isMethod('post') && Session::token() == Request::Input('_token')) {
      ...
      if ($validator->fails()) {
        ...
        if (!empty($data) && isset($data->password) && isset($data->id)) {
          ...
          if (Hash::check(Request::Input('admin_login_password'), $password) && Hash::check(Request::Input('admin_login_password'), $data->password) && isset($get_user_role['user_role_slug']) && ($get_user_role['user_role_slug'] == 'administrator') || ($get_user_role['user_role_slug'] == 'vendor') || ($get_user_role['user_role_slug'] != 'site-user')) {
  ...
  public function postFrontendLogin()
  {
    if (Request::isMethod('post') && Session::token() == Request::Input('_token')) {
      ...
      $messages = [
        'login_email.required'   =>  Lang::get('validation.email_required'),
