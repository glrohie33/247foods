...
class RegisterController extends Controller
{
  ...
  public function redirectToUserRegistrationProcess($ref = null)
  {
    $data  = array();
    ...
    $get_data['ref'] = null;
    if ($ref != null) {
      $user = User::find(base64_decode($ref));
      $get_data['ref'] = $user->email;
    }
      ...
      if (Request::isMethod('post') && Session::token() == Request::input('_token')) {
        ...
        if ($validator->fails()) {
          ...
          if (Role::insert(
            ...
            if (!empty(Request::input('referee'))) {
              $User->referee = Request::input('referee');
            }
            if ($User->save()) {
  ...
  function userVerifyAccount($code)
  {
    if (isset($code) && !empty($code)) {
      ...
      $user->save();
      Session::flash('success-message', 'your account has been successfully confirmed');
   ...
   * @return void
   */
  public function userRegistration($ref = null)
  {
    if (Request::isMethod('post') && Session::token() == Request::Input('_token')) {
      $data = Request::all();
