...
class GetFunction
{
  ...
  public function sendCustomMail($data)
  {
    ...
    if ($data['source'] == 'order_confirmation') {
      ...
      $get_view_data['_view']        =   $view;
      $get_view_data['_mail_to']    =   $data['email'];
      $get_view_data['_unique_code'] = $data['unique_code'];
      $get_view_data['_from'] = 'info@247foods.ng';
    ...
    if (count($get_view_data) > 0) {
      ...
      Mail::send($view, $get_view_data, function ($message) use ($get_view_data) {
        $message->to($get_view_data['_mail_to']);
        $message->subject($get_view_data['_subject']);
        if (array_key_exists('_from', $get_view_data)) {
          $message->from($get_view_data['_from']);
        } else {
          $message->from('info@247foods.ng');
        }
      });
    }
  }
  ...
  public function manage_coupon($coupon_code, $action)
  {
    ...
    if (count($coupon_response) > 0) {
      ...
      } elseif (!empty($coupon_response['coupon_condition_type']) && $coupon_response['coupon_condition_type'] == 'discount_from_shipping') {
        $coupon_set_response = $this->cart->calculationCoupon($coupon_response['coupon_amount'], 'discount_from_shipping', $coupon_code);
        if ($coupon_set_response) {
          $response_str = 'discount_from_shipping';
          return $response_str;
        } else {
          $response_str = 'discount error';
          return $response_str;
        }
      } elseif (!empty($coupon_response['coupon_condition_type']) && $coupon_response['coupon_condition_type'] == 'percentage_discount_from_shipping') {
        $coupon_set_response = $this->cart->calculationCoupon($coupon_response['coupon_amount'], 'percentage_discount_from_shipping', $coupon_code);
        if ($coupon_set_response) {
          $response_str = 'percentage_discount_from_shipping';
          return $response_str;
        } else {
          $response_str = 'discount error';
          return $response_str;
  ...
  public static function getTopCatChildren($topCat)
  {

    return Term::where(['type' => 'product_cat', 'status' => 1])->whereIn('parent', $topCat)->get()->toArray();
  }
