...
class Cart implements CartInterface
{
  ...
  public function getLocalDeliveryShippingPercentageTotal()
  {
    ...
    $total = $this->getSubTotalAndTax()  * ($get_shipping['local_delivery']['delivery_fee'] / 100.0);
    if($total < 800){
      $total = 800;
    }
    return $total;
  ...
  public function getLocalDeliveryShippingPerProductWeight()
  {
    $shipping_details = $this->option->getShippingMethodData();
    $shipping_cost = 0;
    if ($this->items()->count() > 0) {
      foreach ($this->items() as $item) {
        ...
        $item_shipping_amount = $total_weight * $shipping_details['local_delivery']['delivery_fee'];
        $shipping_cost += $item_shipping_amount;
      }
    }
  ...
  public function getShippingMethod()
  {
    ...
    if (!$get_shipping['shipping_option']['enable_shipping'] || ($get_shipping['shipping_option']['enable_shipping'] && !$get_shipping['flat_rate']['enable_option'] && !$get_shipping['free_shipping']['enable_option'] && !$get_shipping['local_delivery']['enable_option'])) {
      ...
      if (!$this->session->has('eBazar_shipping_method')) {
        ...
        } elseif ($get_shipping['local_delivery']['enable_option'] && $get_shipping['local_delivery']['fee_type'] === 'per_weight' && $get_shipping['local_delivery']['delivery_fee']) {

          $this->setShippingMethod(array('shipping_method' => 'local_delivery', 'shipping_cost' => $this->getLocalDeliveryShippingPerProductWeight()));
          if ($this->session->has('eBazar_shipping_method')) {
            return $this->session->get('eBazar_shipping_method');
          }
        }
        ...
        } elseif ($get_shipping['local_delivery']['enable_option'] && $get_shipping['local_delivery']['fee_type'] === 'per_weight' && $get_shipping['local_delivery']['delivery_fee'] && isset($data['shipping_method']) && $data['shipping_method'] == 'local_delivery') {
          $this->setShippingMethod(array('shipping_method' => 'local_delivery', 'shipping_cost' => $this->getLocalDeliveryShippingPerProductWeight()));
          if ($this->session->has('eBazar_shipping_method')) {
            return $this->session->get('eBazar_shipping_method');
          }
        } else if ($get_shipping['local_delivery']['enable_option'] && $get_shipping['local_delivery']['fee_type'] === 'cart_total' && $get_shipping['local_delivery']['delivery_fee'] && isset($data['shipping_method']) && $data['shipping_method'] == 'local_delivery') {
          $this->setShippingMethod(array('shipping_method' => 'local_delivery', 'shipping_cost' => $this->getLocalDeliveryShippingPercentageTotal()));
  ...
  public function calculationCoupon($amount, $type, $coupon_code)
  {
    ...
    } elseif ($type == 'discount_from_shipping') {
      $get_val = $amount;
    } elseif ($type == 'percentage_discount_from_shipping') {
      $get_val = $this->getShippingCost() * ($amount / 100);
    }
    ...
    if ($get_val && $get_val > 0 && $this->getTotal() > $get_val) {
      ...
      if ($this->session->has('applyed_coupon_type')) {
        $this->session->remove('applyed_coupon_type');
        $this->session->set('applyed_coupon_type', $type);
      } else {
        $this->session->set('applyed_coupon_type', $type);
      }
    } else {
  ...
  public function  couponType()
  {
    ...
    if ($this->session->has('applyed_coupon_type')) {
      $price = $this->session->get('applyed_coupon_type');
    }
